...
router.get('/:businessId', authMiddleware, async (req, res) => {
  ...
  const [ordersToday, revenueToday, recentOrders, avgRating] = await Promise.all([
    query(
      `SELECT COUNT(*) AS count FROM orders WHERE business_id=$1 AND created_at::date=$2 AND status != 'cancelled'`,
      [bid, today]
    ),
    query(
      `SELECT COALESCE(SUM(total),0) AS revenue FROM orders WHERE business_id=$1 AND created_at::date=$2 AND status != 'cancelled'`,
      [bid, today]
    ),
    query(
